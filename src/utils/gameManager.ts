...
export class GameManager {
  ...
  public startGame(): boolean {
    ...
    this.gameState.players.forEach((player, index) => {
      ...
      player.team = ROLE_CONFIG[roles[index]].team;
      player.voteHistory = [];
  ...
  public getVisiblePlayers(playerId: string): Partial<Player>[] {
    const player = this.gameState.players.find(p => p.id === playerId);
    if (!player?.role) return [];

    return this.gameState.players.map(p => {
      const visibleInfo: Partial<Player> = {
        id: p.id,
        name: p.name,
        isLeader: p.isLeader,
        revealed: p.revealed
      ...
      if (p.revealed || (player.role && p.role && this.shouldSeeRole(player.role, p.role))) {
        visibleInfo.role = p.role;
        visibleInfo.team = p.team;
      }
...
(this.gameState as any).winningTeam = winningTeam;
  ...
  private shouldSeeRole(viewerRole: PlayerRole, targetRole: PlayerRole): boolean {
    if (!viewerRole || !targetRole) return false;
    
    const viewer = ROLE_CONFIG[viewerRole];
    const target = ROLE_CONFIG[targetRole];

    if (viewerRole === 'Merlin' && target.team === 'EVIL' && targetRole !== 'Mordred') {
      return true;
    }

    if (viewer.team === 'EVIL' && target.team === 'EVIL' && targetRole !== 'Oberon') {
      return true;
    }
